...
class _SociordDraggableSheetState extends State<SociordDraggableSheet> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (widget.onOpen != null) {
        debugPrint('SociordDraggableSheet: onOpen called');
        widget.onOpen!();
      }
    });
  ...
  void _handleSheetChange() {
    ...
    if (size <= widget.minChildSize + 0.01 && _isOpen) {
      ...
      if (widget.onClose != null) {
        debugPrint('SociordDraggableSheet: onClose called (handleSheetChange)');
        widget.onClose!();
      }
    } else if (size > widget.minChildSize + 0.01 && !_isOpen) {
      _isOpen = true;
      if (widget.onOpen != null) {
        debugPrint('SociordDraggableSheet: onOpen called (handleSheetChange)');
        widget.onOpen!();
      }
    }
    _lastSize = size;
  ...
  void dispose() {
    ...
    _controller.dispose();
    if (!_isOpen && widget.onClose != null) {
      debugPrint('SociordDraggableSheet: onClose called (dispose)');
      widget.onClose!();
    }
    super.dispose();
  }
